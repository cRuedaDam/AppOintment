...
import com.example.reservapp.databinding.ActivityCustomerSignUpBinding
            ...
            MainScope().launch(Dispatchers.IO) {
                firebaseAuth.createUserWithEmailAndPassword(userEmail, userPassword)
                    .addOnCompleteListener {
                        if (it.isSuccessful) {
                            val user = FirebaseAuth.getInstance().currentUser // Apunta al usuario
                            var userRef = db.collection("users")
                                .document(user!!.uid) //Referencia a al BBDD en la coleccion users
